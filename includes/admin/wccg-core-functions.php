...
// 			$wpdb->query( $wpdb->prepare( "UPDATE $wpdb->posts SET guid=%s WHERE ID=%d", get_permalink( $coupon_id ), $coupon_id ) ); // Slow
	...
	$meta_array = apply_filters( 'woocommerce_coupon_generator_coupon_meta_data', array(
		...
		// Additional fields from WooCommerce Subscriptions/etc - should these be conditional?
		'_wcs_number_payments'       => empty( $args['wcs_number_payments'] ) ? '' : absint( $args['wcs_number_payments'] ),
		'affwp-woo-coupon'           => empty( $args['affwp-woo-coupon'] ) ? '' : wc_clean( $args['affwp-woo-coupon'] ),
